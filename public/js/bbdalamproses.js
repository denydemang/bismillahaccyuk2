...
function resetawal(){
    ...
    $('#btnsimpanbbproses').addClass('btnsimpanbbproses');
    $('#btnsimpanproses').html('Simpan');
    $('.judulmodal').html('Tambah Bahan Baku');
...
$(document).ready(function(){
    ...
    $('.formbbproses').submit(function(e){
        e.preventDefault();
        let idbelibahan = $('#btnsimpanbbproses').data('idbelibahan');
        if ($('#btnsimpanbbproses').hasClass("btnsimpanbbproses")){
            $.ajax({
                ...
                beforeSending: function(){
                    ...
                    $('#btnsimpanbbproses').html('Menyimpan..')
                },
                complete :function(){
                    $('#btnsimpanbbproses').removeAttr('disable')
                    $('#btnsimpanbbproses').html('<i class="fas fa-save"></i> Simpan')
                },
                
                success: function(response) {

                if (response.errors){
                        if (response.errors.namabahan){
                            $('.namabahan').addClass('is-invalid')
                            $('.namabahaninvalid').html(response.errors.namabahan);
                        } else {
                            $('.namabahan').removeClass('is-invalid')
                        ...
                        if (response.errors.harga){
                            ...
                            $('.hargainvalid').html(response.errors.harga);
                        } else {
                            $('.harga').removeClass('is-invalid')
                            $('.hargainvalid').html('');
                        }
            ...
            $.ajax({
                url :base_url+'DashboardKelolaProyek/updatebbproses/'+idbelibahan,
                data : $(this).serialize(),
                type: 'post',
                dataType : "json",
                beforeSending: function(){
                    $('#btnsimpanbbproses').attr('disable','disabled')
                    $('#btnsimpanbbproses').html('Mengubah..')
                },
                complete :function(){
                    $('#btnsimpanbbproses').removeAttr('disable')
                    $('#btnsimpanbbproses').html('<i class="fas fa-save"></i> Simpan')
                ...
                if (response.errors){
                    ...
                    if (response.affected >= 1){
                        Swal.fire({
                            ...
                            title: 'Id Beli '+response.notifidbeli+' Dengan Nama Bahan '+response.notifnamabahan+' Telah Berhasil Diupdate',
                            showConfirmButton: false,
                            timer: 3000
                            })
                            tampiltableproses();
